With activity_status_changes as(
    ...
    master.status."dateCreated", geo.user_position.latitude, geo.user_position.longitude, geo.user_position.accuracy,
    ...
    WHERE (master.status."dateCreated"::date BETWEEN ('2024-11-01 00:00:00.000'::date) and ('2024-11-15 23:59:00.000'::date)  AND master.status.status != 'DELETED')
        ...
        geo.user_position.accuracy, geo.address."address", geo.user_position."clientIp"
    ),
    comment_changes as (
        ...
        'Commented' as "action",
        master.comment."dateCreated", geo.user_position.latitude, geo.user_position.longitude, geo.user_position.accuracy,
         geo.user_position."clientIp",
        "geo"."address"."address"
        FROM master.comment
    ...
    agent_actions as (
        SELECT DISTINCT  sales.agent_action."Id", sales.agent_action."userId", sales.agent_action."Id" as "itemId", 'Agent Activity' as "itemType",
                ...
                CASE
                    ...
                    ELSE 'Unknown action'
                END as "action",
                                sales.agent_action."dateCreated", geo.user_position.latitude, geo.user_position.longitude, geo.user_position.accuracy,
                geo.user_position."clientIp",
                "geo"."address"."address"
    ...
    agent_log as (
        SELECT DISTINCT  geo.user_position."Id", geo.user_position."userId", geo.user_position."entityId" as "itemId", 'Agent Activity' as "itemType",
                            ...
                            geo.user_position."dateCreated", geo.user_position.latitude, geo.user_position.longitude, geo.user_position.accuracy,
                            geo.user_position."clientIp",
                            "geo"."address"."address"
        from
        geo.user_position
